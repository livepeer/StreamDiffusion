...
class PreprocessingOrchestrator:
    ...
    def _start_next_frame_preprocessing(self,
                                      ...
                                      stream_width: int,
                                      stream_height: int,
                                      processing_type: str = "controlnet") -> None:
        """Start preprocessing for next frame in background thread"""
        # Handle IPAdapter processing (no scales check needed)
        if processing_type == "ipadapter":
            if not preprocessors:
                self._next_frame_future = None
                return
            
            # Set processing mode for background thread
            self._current_processing_mode = "embedding"
            
            # Submit background processing for IPAdapter
            self._next_frame_future = self._executor.submit(
                self._process_frame_background,
                control_image,
                preprocessors,
                stream_width,
                stream_height
        ...
        # Clear processing mode for ControlNet
        self._current_processing_mode = "controlnet"
        
        # Submit background processing for ControlNet
        self._next_frame_future = self._executor.submit(
            self._process_frame_background,
            control_image,
            preprocessors,
    ...
    def process_pipelined(self, 
        ...
        Args:
            ...
            scales: List of conditioning scales (ControlNet only)
        ...
        Returns:
            Processing results
        """
        # Check if sync processing is required (pipeline-aware preprocessors)
        if self._should_use_sync_processing(preprocessors, scales, stream_width, stream_height):
            return self.process_sync(control_image, preprocessors, scales, stream_width, stream_height, None, processing_type)
        ...
        self._start_next_frame_preprocessing(control_image, preprocessors, scales, stream_width, stream_height, processing_type)
        
        # Apply current frame processing results if available; otherwise signal no update
        return self._apply_current_frame_processing(preprocessors, scales)
    ...
    def _check_pipeline_aware_cached(self, preprocessors: List[Optional[Any]]) -> bool:
        ...
        try:
            ...
            # Additional fallback: check for temporal processors if they exist
            try:
                from .processors.temporal_net import TemporalNetPreprocessor
                for prep in preprocessors:
                    if isinstance(prep, TemporalNetPreprocessor):
                        self._has_feedback_cache = True
                        break
            except ImportError:
                pass
            ...
            if not self._has_feedback_cache:
                for prep in preprocessors:
                    if prep is not None:
    ...
    def _process_single_preprocessor_group(self,
        ...
        try:
            ...
            # PIL processing fallback - use thread-safe key
